  ...
  return (
    <div className="h-full overflow-auto px-5 py-6">
      ...
      <p className="mt-1 text-sm text-muted">
        Listener health and local email / webhook configuration.
      </p>
      ...
      <section className="mt-6 max-w-2xl rounded-[24px] border border-border bg-surface px-5">
        ...
        <StatusRow
          label="Status"
          value={smtp?.running ? "Running" : "Stopped"}
          ok={smtp?.running}
        />
      ...
      <section className="mt-4 max-w-2xl rounded-[24px] border border-border bg-surface px-5">
        ...
        <StatusRow
          label="Status"
          value={http?.running ? "Running" : "Stopped"}
          ok={http?.running}
        />
      ...
      <section className="mt-4 max-w-2xl rounded-[24px] border border-border bg-surface p-5">
        ...
        <pre className="mt-4 overflow-auto rounded-[16px] bg-surface-2 p-4 font-mono text-xs">
          {snippet}
        </pre>
        <div className="mt-3">
      ...
      <section className="mt-4 max-w-2xl rounded-[24px] border border-border bg-surface p-5">
        ...
        <p className="mb-4 mt-1 text-sm text-muted">
          This proves ingest, parsers, and the dashboard. It is not an SMTP health check — that is
          the listener status above. Messages caught here are marked source=test.
        </p>
        <TestEmailForm />
      </section>
    </div>
