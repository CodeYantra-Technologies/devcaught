...
function needsSupportedNode(command, args) {
  if (command === "vite") return true;
  const script = args[0] ?? "";
  return command === "node" && typeof script === "string" && script.includes("vite");
}

function assertSupportedNodeForCommand(command, args) {
  if (!needsSupportedNode(command, args) || isSupportedNode()) return;
  console.error(
    [
      `DevCaught requires Node.js ${MIN_NODE_VERSION} or later (found v${process.versions.node}).`,
...
function main(argv) {
  ...
  assertSupportedNodeForCommand(command, args);
